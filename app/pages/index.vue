  ...
  <div class="min-h-screen bg-gray-50 flex flex-col items-center py-10">
    ...
    <div class="w-full max-w-2xl bg-white rounded-xl shadow-lg overflow-hidden flex flex-col h-[600px]">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-4" ref="chatContainer">
        ...
        <div v-for="msg in messages" :key="msg._tick ? `${msg.id}-${msg._tick}` : msg.id"
             class="flex flex-col" 
...
marked.setOptions({
  highlight: function(code, lang) {
    ...
    // 2. 尝试高亮
    try {
      // 使用 highlight 而不是 getLanguage 来判断，更稳妥
      return hljs.highlight(code, { 
        language: lang || 'plaintext', 
        ignoreIllegals: true // ✅ 关键：忽略非法字符，防止代码块没写完时报错
      }).value;
    } catch (e) {
      // 如果高亮失败，返回原代码
      return code;
